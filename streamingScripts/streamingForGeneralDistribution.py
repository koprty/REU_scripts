...
try:
    ...
    consumer_secret  = APP_SECRETS[0]
    access_token = OAUTH_TOKENS[0]
    access_token_secret = OAUTH_TOKEN_SECRETS[0] 
  
    auth = tweepy.OAuthHandler(consumer_key, consumer_secret)
    auth.set_access_token(access_token, access_token_secret)
    api = tweepy.API(auth)
    keyword_list = ['election', 'political parties', 'political party', 'electorial college', 'nomination convention', 'Republican Party', 'Democractic Party', 'United States representatives']
...
class DabStreamListener (tweepy.StreamListener):
    ...
    def __init__(self): 
        self.database = sqlite3.connect('rt_tweets.sqlite', timeout = 20)
    ...
    def on_data(self,data):
        try:
           #create string for sql execution
            ...
            sql_exec_string = """INSERT INTO GENERALTWEETS_STREAMING (
            Tweet_ID, 
            Longitude,  
            Latitude,         
            Lang,         
            ...
            " "+str(utc)+");"  #remember to change this in according to your query!!!!!          
...
print ("hi - starting streaming file")
